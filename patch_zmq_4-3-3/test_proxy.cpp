...
//    sprintf (endpoint_source, "inproc://endpoint%d", databag->id);
    snprintf (endpoint_source, 256, "inproc://endpoint%d", databag->id);
...
//    sprintf (routing_id, "%04X-%04X", rand () % 0xFFFF, rand () % 0xFFFF);
    snprintf (routing_id, ROUTING_ID_SIZE, "%04X-%04X", rand () % 0xFFFF, rand () % 0xFFFF);
...
//            sprintf (content, "request #%03d", ++request_nbr); // CONTENT_SIZE
            snprintf (content, CONTENT_SIZE_MAX, "request #%03d", ++request_nbr); // CONTENT_SIZE
            if (is_verbose)
...
//        sprintf (endpoint_source, "inproc://endpoint%d", i);
        snprintf (endpoint_source, 256, "inproc://endpoint%d", i);
        TEST_ASSERT_SUCCESS_ERRNO (
